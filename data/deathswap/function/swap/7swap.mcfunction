...
execute as @a[sort=random,limit=1,tag=!swap1,tag=!swap2,tag=!swap3,tag=!swap4,tag=!swap5,tag=!out] run tag @s add swap6
execute as @a[tag=!swap1,tag=!swap2,tag=!swap3,tag=!swap4,tag=!swap5,tag=!swap6] run tag @s add swap7
...
execute at @a[tag=swap7] run summon armor_stand ~ ~ ~ {Invisible:1b,Invulnerable:1b,NoGravity:1b,Tags:["swapper7"]}
...
tp @a[tag=swap6] @e[tag=swapper7,limit=1]
tp @a[tag=swap7] @e[tag=swapper1,limit=1]
